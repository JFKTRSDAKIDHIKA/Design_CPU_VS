...
| `state4` | `101` | FETCH_SECOND_WORD | B/C 类双字指令取第二字（立即数 / reserved） |
| `state5` | `111` | EXECUTE_DOUBLE | B 类双字指令的执行拍 ⇒ **退休** |
| `state6` | `010` | SAVE_RETURN_ADDRESS | C 类（CALLA）保存返回地址到 `R15` |
| `state7` | `110` | LOAD_CALL_TARGET | C 类（CALLA）将 `PC+signext(rel8)` 写入 PC ⇒ **退休** |
...
| `011 ALU_IN_BR`     | `imm8` (符号扩展) | `PC` | 相对分支 / CALLA |
...
#### B 类（双字，`IR[15]=1` 且 `IR[15:8]` 属于 `80/81/82/83/84/85`）
...
| 助记符 | Opcode | 第 1 字 | 第 2 字 | 语义 |
|---|---|---|---|---|
| **`CALLA target`** | **`F0`** | `11110000 oooooooo` | `reserved` | **`R15 ← PC_after_calla; PC ← PC_after_calla + signext(off8)`，flags 保持** |

`R15` 写入的是 *CALLA 后续指令地址*（即 `addr_calla + 2`），也是相对跳转基址；
第二字当前完全忽略，仅保留双字格式。汇编器接受数值或 label 目标并自动计算
signed rel8，范围为 `[-128, 127]`。

#### 单字控制流补充

| 助记符 | Opcode | 机器码 | 语义 |
|---|---|---|---|
| **`RET`** | **`F1`** | **`11110001 0000 0000`** | **`PC ← R15`，flags 保持** |
...
- **C 类**（`CALLA`）：唯一一条多拍控制流指令，需要 *先写 R15、再写 PC*
  两步顺序写回；第二字 reserved 仅维持双字格式，目标由 `PC_after_calla`
  加 signed rel8 得到。
...
| 命令 | 作用 |
|---|---|
| `python3 model/assembler.py <asm> --hex-out <hex> --layout-out <layout>` | 汇编（已支持 NOT/ASR/ADDI/ANDI/CALLA/RET） |
| `python3 model/reference_model.py --hex <hex> --trace-out … --summary-out …` | Python 参考模型（UVM oracle，支持 A/B/C 扩展） |
...
ADDI  dr, imm16
ANDI  dr, imm16
CALLA target            ; target 可以是数值或 label，汇编为signed rel8
RET
```
...
- **无随机扩展指令 fuzzing**：差分流水当前是定向长测试；可与
  ...
  扩展为带 NOT/ASR/ADDI/ANDI/CALLA/RET 的随机 fuzzing 回归。
